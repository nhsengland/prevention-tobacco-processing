%md
**Tobacco Data Processing Pipeline**
%run ./config
### Read and prepare patient level data
### ===================================
...
sql_query = """select RecordID
                    ,AuditID
                    ,REPORTING_PERIOD
                    ,case when b.Org_Code_Current is not null then b.Org_Code_Current else a.ODS_Code end as ODS_CODE
                    ,ODS_Name
                    ,SETTING_TYPE
                    ,ACTIVITY_TYPE
                    ...
                    ,UDALDay
                from df_pld a 
                left join (select distinct Org_Code
                                    ,Org_Code_Current
                                    from df_prov_ref a 
                                ) b on a.ODS_Code = b.Org_Code
...
df_ref_subicb = df_ref_subicb.filter(df_ref_subicb.Effective_To.isNull())
...
from df_pld a 
left join (select * from  df_ref_subicb) b on a.CCG_OF_REGISTRATION = b.Organisation_Code
left join (select * from  df_ref_subicb) c on a.CCG_OF_RESIDENCE = c.Organisation_Code
left join (select * from  df_ref_subicb) d on a.Der_Postcode_CCG_Code = d.Organisation_Code"""
...
sql_query = """
    ...
    from (
        select distinct ODS_Code
                    ,case when ODS_Code = '902' then 'CORNWALL COUNCIL LOCAL AUTHORITY' 
                          when ODS_Code = '8J558' then 'FIRST CONTACT CLINICAL'
                          else Organisation_Name end as ODS_NAME
                    ,case when ODS_Code = '902' then 'QT6' 
                          when ODS_Code = '8J558' then 'QHM'
                          when ODS_Code = 'RV3' then 'QRV'
                          when ODS_Code = 'RVN' then 'QUY'
                          when ODS_Code = 'RWX' then 'QU9'
                          when ODS_Code = 'RXK' then 'QUA'
                          else b.High_Level_Health_Authority_Code end as ICB_Code
                    ,case when ODS_Code = '902' then 'Y58' 
                          when ODS_Code = '8J558' then 'Y63'
                          else b.National_Grouping_Code end as Region_Code
        from df_pld a
        left join df_ref_prov b on a.ODS_CODE = b.Organisation_Code
    ) a 
    left join df_ref_icb b on a.ICB_Code = b.ICB_Code
    left join df_ref_region c on a.Region_Code = c.Region_Code"""

# execute sql 
df_ref_prov_out = spark.sql(sql_query)
df_ref_prov_out.createOrReplaceTempView("df_ref_prov_out");

display(df_ref_prov_out)

# write data
df_ref_prov_out.coalesce(1).write.mode('overwrite').options(header='True', delimiter=',').csv(ref_prov_path)
### generate commissioner reference data
### ====================================
...
sql_query = """select distinct a.Organisation_Code
                ,a.Organisation_Name
                ,a.Organisation_Code_Name
                ,a.STP_Code as ICB_Code
                ,a.STP_Name as ICB_Name
                ,a.STP_Code_Name as ICB_Code_Name
                ,d.ICB_Map
                ,a.Region_Code
                ,c.Region as Region_Name
                ,a.Region_Code_Name
...
# df_msds = df_msds.filter("AntenatalAppDate <= '" + max_date_ip + "'")
...
# df_mhsds = df_mhsds.filter("StartDateHospProvSpell <= '" + max_date_ip + "'")

# create temp table to query
df_mhsds.createOrReplaceTempView("df_mhsds");
### read required SUS data
### ======================
...
# df_sus = df_sus.filter("Admission_Date <= '" + max_date_ip + "'")
...
StructField('Service_Setting', StringType(), True),
...
for index, row in df_loop.iterrows():
    ...
    sql_query = "select " + str(element_id) + " as Element_ID \
                    ...
                    ,case \
                        when Setting_Type in ('1','2') then 'All other services' \
                        when Setting_Type = '3' then 'Maternity services' \
                        else 'All other services' end as Service_Setting \
                    ,case \
                        ...
                        when Setting_Type = '2' and Activity_Type in ('1','3') then 'Mental Health Inpatient' \
                        when Setting_Type = '2' and Activity_Type = '4' then 'Specialist Community Mental Health' \
                        when Setting_Type = '3' then 'Maternity' \
                        else 'Other activity' end as Activity_Type \
                    ,case \
...
# write raw df_elements_out table for HI matrix usage 
df_elements_out_pld.coalesce(1).write.mode('overwrite').options(header='True', delimiter=',').csv(element_hi_path)

# reformat data into long structure and calculate total group
df_elements_total = df_elements_out_pld.groupBy("Element_ID","Date","Provider_Code","Commissioner_Code","Service_Setting","Activity_Type").sum("Value") \
    ...
    .select("Element_ID","Date","Provider_Code","Commissioner_Code","Service_Setting","Activity_Type","Health_Inequalities_Group","Health_Inequalities_SubGroup","Value")

df_elements_gender = df_elements_out_pld.groupBy("Element_ID","Date","Provider_Code","Commissioner_Code","Service_Setting","Activity_Type","Gender").sum("Value") \
    .withColumn("Health_Inequalities_Group", lit("Gender")) \
    .withColumnRenamed("Gender","Health_Inequalities_SubGroup") \
    .withColumnRenamed("sum(Value)","Value") \
    .select("Element_ID","Date","Provider_Code","Commissioner_Code","Service_Setting","Activity_Type","Health_Inequalities_Group","Health_Inequalities_SubGroup","Value")

df_elements_age = df_elements_out_pld.groupBy("Element_ID","Date","Provider_Code","Commissioner_Code","Service_Setting","Activity_Type","Age").sum("Value") \
    .withColumn("Health_Inequalities_Group", lit("Age")) \
    .withColumnRenamed("Age","Health_Inequalities_SubGroup") \
    .withColumnRenamed("sum(Value)","Value") \
    .select("Element_ID","Date","Provider_Code","Commissioner_Code","Service_Setting","Activity_Type","Health_Inequalities_Group","Health_Inequalities_SubGroup","Value")

df_elements_eth = df_elements_out_pld.groupBy("Element_ID","Date","Provider_Code","Commissioner_Code","Service_Setting","Activity_Type","Ethnicity").sum("Value") \
    .withColumn("Health_Inequalities_Group", lit("Ethnicity")) \
    .withColumnRenamed("Ethnicity","Health_Inequalities_SubGroup") \
    .withColumnRenamed("sum(Value)","Value") \
    .select("Element_ID","Date","Provider_Code","Commissioner_Code","Service_Setting","Activity_Type","Health_Inequalities_Group","Health_Inequalities_SubGroup","Value")

df_elements_ethgroup = df_elements_out_pld.groupBy("Element_ID","Date","Provider_Code","Commissioner_Code","Service_Setting","Activity_Type","Ethnic_Group").sum("Value") \
    .withColumn("Health_Inequalities_Group", lit("Ethnic_Group")) \
    .withColumnRenamed("Ethnic_Group","Health_Inequalities_SubGroup") \
    .withColumnRenamed("sum(Value)","Value") \
    .select("Element_ID","Date","Provider_Code","Commissioner_Code","Service_Setting","Activity_Type","Health_Inequalities_Group","Health_Inequalities_SubGroup","Value")

df_elements_imd = df_elements_out_pld.groupBy("Element_ID","Date","Provider_Code","Commissioner_Code","Service_Setting","Activity_Type","IMD_Quintile").sum("Value") \
    .withColumn("Health_Inequalities_Group", lit("IMD_Quintile")) \
    .withColumnRenamed("IMD_Quintile", "Health_Inequalities_SubGroup") \
    .withColumnRenamed("sum(Value)", "Value") \
    .select("Element_ID","Date","Provider_Code","Commissioner_Code","Service_Setting","Activity_Type","Health_Inequalities_Group","Health_Inequalities_SubGroup","Value")

df_elements_out = df_elements_total.union(df_elements_gender) \
    .union(df_elements_age) \
    .union(df_elements_eth) \
    .union(df_elements_ethgroup) \
    .union(df_elements_imd)
    
#df_elements_out = df_elements_out.union(df_elements_total)
...
sql_query_ip = """select '001' as Element_ID
                ...
                ,a.ODS_CODE as Provider_Code
                ,NULL as Commissioner_Code
                ,'All other services' as Service_Setting
                ,'Acute Inpatient' as Activity_Type
                ,'Total' as Health_Inequalities_Group
                ,'Total' as Health_Inequalities_SubGroup
                ,SUM(COMPLETESMOKINGSTATUS_IP) as Value
        from df_agg a 
        inner join (select distinct ODS_Code from df_ref_prov) b on a.ODS_Code = b.ODS_Code
...
sql_query_mat = """select '001' as Element_ID
                ...
                ,a.ODS_CODE as Provider_Code
                ,NULL as Commissioner_Code
                ,'Maternity services' as Service_Setting
                ,'Maternity' as Activity_Type
                ,'Total' as Health_Inequalities_Group
                ,'Total' as Health_Inequalities_SubGroup
...
sql_query_op = """select '001' as Element_ID
                ...
                ,NULL as Commissioner_Code
                ,'All other services' as Service_Setting
                ,'Acute Outpatient' as Activity_Type
                ,'Total' as Health_Inequalities_Group
                ,'Total' as Health_Inequalities_SubGroup 
...
### exclude acute community for time being
# define sql query
...
sql_query_com_mh = """select '001' as Element_ID
                ...
                ,NULL as Commissioner_Code
                ,'All other services' as Service_Setting
                ,'Specialist Community Mental Health' as Activity_Type
                ,'Total' as Health_Inequalities_Group
                ,'Total' as Health_Inequalities_SubGroup
...
sql_query_mh = """select '001' as Element_ID
                ...
                ,NULL as Commissioner_Code
                ,'All other services' as Service_Setting
                ,'Mental Health Inpatient' as Activity_Type
                ,'Total' as Health_Inequalities_Group
                ,'Total' as Health_Inequalities_SubGroup
...
sql_query_sus = """select '010' as Element_ID 
        ...
        and Der_Admit_Treatment_Function_Code not in ('424','501','560')
...
df_elements_out.createOrReplaceTempView("df_elements_out");
...
### prepare tableau metric table
### ============================

# aggregated metrics data
df_metrics_agg = spark.read.format("csv").option("header","true").load(metric_aggregate_path)

# limit to remove HI groups
df_metrics_agg = df_metrics_agg.filter("Health_Inequalities_Group = 'Total'")
...
df_ref_reg = spark.read.format("csv").option("header","true").load(ref_region_path)
...
df_ref_prov.createOrReplaceTempView("df_ref_prov");
df_ref_com.createOrReplaceTempView("df_ref_com");
df_ref_reg.createOrReplaceTempView("df_ref_reg");
df_ref_icb.createOrReplaceTempView("df_ref_icb");

sql = """select * from df_ref_com"""
test = spark.sql(sql)
display(test)
### calculate quartiles 
...
sql_query = """select dat.Metric_ID
        ...
        ,case when dat.Level = 'Provider' then ref_prov.ODS_NAME
            when dat.Level = 'Commissioner' then ref_com.Organisation_Name
        ...
        ,case when dat.Level = 'Provider' then ref_prov.Region_Name
            when dat.Level = 'Commissioner' then ref_com.Region_Name
        ...
        ,case when dat.Level = 'Provider' then ref_prov.Region_Map
            when dat.Level = 'Commissioner' then ref_com.Region_Map
...
left join (
    ...
    and dat.Service_Setting = Q1.Service_Setting
    and dat.Activity_Type = Q1.Activity_Type
...
left join (
    ...
    and dat.Service_Setting = Q2.Service_Setting
    and dat.Activity_Type = Q2.Activity_Type
...
left join (
    ...
    and dat.Service_Setting = Q3.Service_Setting
    and dat.Activity_Type = Q3.Activity_Type
...
left join (
    ...
    and dat.Service_Setting = Q4.Service_Setting
    and dat.Activity_Type = Q4.Activity_Type
...
where dat.Activity_Type = case when ref_metrics.Metric_Setting = 'Maternity' then ref_metrics.Metric_Setting 
                                else dat.Activity_Type end
and dat.Service_Setting is not null
and dat.Activity_Type is not null 
and dat.Date is not null 
and dat.Org_Code is not null
...
## Apply date filters to settings

df_tableau = df_tableau.filter("Date >= '" + min_date + "'")

df_tableau_mat = df_tableau.filter("Service_Setting = 'Maternity services'")
df_tableau_mat = df_tableau_mat.filter("Date <= '" + max_date_maternity + "'")

df_tableau_ip = df_tableau.filter("Service_Setting <> 'Maternity services'")
df_tableau_ip = df_tableau_ip.filter("Date <= '" + max_date_ip + "'")

df_tableau = df_tableau_ip.union(df_tableau_mat)

## create base ICB table 

## split out ICB data
df_tableau_icb = df_tableau.filter("Level = 'ICB'")
df_tableau_icb.createOrReplaceTempView("df_tableau_icb");

sql = """select base.Metric_ID
                ,concat(base.Metric_ID, ': ', base.Metric_Name) as Metric_Name
                ,base.Metric_Category
                ,base.Metric_Type
                ,base.Date
                ,base.Service_Setting
                ,base.Activity_Type
                ,base.Aggregation_Source
                ,'Total' as Health_Inequalities_Group
                ,'Total' as Health_Inequalities_SubGroup
                ,'ICB' as Level
                ,base.Org_Code
                ...
                ,base.Region_Map
                ,dat.Numerator
                ,base.Numerator_Description
                ,base.Numerator_Source
                ,base.Numerator_Logic
                ,dat.Numerator_Suppressed_Flag
                ,dat.Denominator
                ,base.Denominator_Description
                ,base.Denominator_Source
                ,base.Denominator_Logic
                ,dat.Denominator_Suppressed_Flag
                ,dat.Lower_Quartile
                ,dat.Upper_Quartile
                ,dat.Min_Value
                ,dat.Max_Value
         ...
         from 
         (
            select distinct icb.ICB_Code as Org_Code
            ,icb.ICB_Name as Org_Name
            ,icb.ICB_Code
            ,icb.ICB_Name
            ,icb.ICB_Map
            ,com.Region_Code
            ...
            ,Metric_Category
            ,Metric_Type
            ,Numerator_Element_Description as Numerator_Description
            ,Numerator_Element_Source as Numerator_Source
            ,Numerator_Element_Logic as Numerator_Logic
            ,Denominator_Element_Description as Denominator_Description
            ,Denominator_Element_Source as Denominator_Source
            ,Denominator_Element_Logic as Denominator_Logic
            ,Date
            ,Aggregation_Source
            ,Service_Setting
            ,Activity_Type
            from df_ref_icb icb 
            left join df_ref_com com on icb.ICB_Code = com.ICB_Code
            cross join (select * from df_ref_metrics where Reporting_Flag = '1') metrics
            cross join (select distinct Date, Aggregation_Source, Service_Setting, Activity_Type, Health_Inequalities_Group, Health_Inequalities_SubGroup from df_tableau_icb) tableau

         ) base 
         left join df_tableau_icb dat 
            on base.Metric_ID = dat.Metric_ID
                 and base.Date = dat.Date
                 and base.Service_Setting = dat.Service_Setting
                 and base.Activity_Type = dat.Activity_Type
                 and base.Aggregation_Source = dat.Aggregation_Source
                 and base.Org_Code = dat.Org_Code"""

df_tableau_icb = spark.sql(sql)

## join original to new ICB and Region datta
df_tableau = df_tableau.filter("Level <> 'ICB'")

df_tableau = df_tableau.union(df_tableau_icb)

# write data
df_tableau.coalesce(1).write.mode('overwrite').options(header='True', delimiter=',').csv(tableau_metric_path)
### to build tableau dq table
### =========================
...
sql_query = "select distinct Org_Code_Current as Org_Code, Setting_Type, Specified_ICB from df_ref_prov_expctd where DQ_Flag = '1'"
df_ref_prov_expctd = spark.sql(sql_query)

display(df_ref_prov_expctd)
...
sql_query = """select distinct Reporting_Period			
                        ...
                        ,case when Specified_ICB is not null then Specified_ICB else High_Level_Health_Authority_Code end as ICB_Code
                        ,Setting_Type	
...
sql_query = """select 	base.Reporting_Period
                ...
                case when record.Percent_LTFU <= 0.75 then 1 else 0 end + 
...
sql_query = """select Reporting_Period
        ...
        select Reporting_Period
                ...
                ,case when Percent_CarePlan_Complete >= 0.9 then 'Pass' else 'Fail' end as DQ_Check
        from df_wide
        ...
        select Reporting_Period
                ...
                ,case when Percent_LTFU <= 0.75 then 'Pass' else 'Fail' end as DQ_Check
        from df_wide
